...
//   useEffect(() => {
//     // whenever the market id exists/changes, refresh DB state
...
//   useEffect(() => {
//     // if DB row exists (GET succeeded), consider it inserted and hide Add2Database
...
//       // update parent UI state so match displays ID immediately
//       onMarketCreated?.(match, createdIdStr);

//       setAdminStatus(`✅ Market created (ID: ${createdIdStr}).`);
//       setDbStatus("");
//       setDbInserted(false); // until we confirm DB insert
//       setShowAdminPopup(false);
//       setAdminAnimatingIn(false);

//       // refresh DB state (in case it already exists)
...
//     // Must match MatchSchema on server
//     const bodyObj = {
...
//       setDbInserted(true); // ✅ hide Add2Database button after success
...
//   // NOTE: server must implement PATCH /api/matches/[marketId] to update request_settlement_hash
//   // =====================================================================
...
//     // ✅ Requires you to add PATCH handler on server for this path
//     const method = "PATCH";
//     const path = `/api/matches/${effectiveMarketId}`;

//     // minimal body — server will update request_settlement_hash
//     const bodyObj = {
//       requestSettlementHash: settleTxHash,
//       note: "Settlement requested",
//     };
//     const rawBody = JSON.stringify(bodyObj);
...
//   // show Add2Database only for admin AND after createMarket confirmed AND not already inserted
//   const showAdd2DatabaseButton =
//     isAdmin && adminReceipt.isSuccess && !!createdMarketIdFromLogs && !dbInserted;

//   // Settlement buttons:
//   // - if settlement not requested onchain: show Request Settlement (onchain)
//   // - after requestSettlement tx confirmed: show addSRToDatabase until DB has request_settlement_hash
...
//   // Admin note: if DB has request_settlement_hash and DB says settled is false
//   const dbSettled =
//     dbMatch?.settled === true;
...
//         {/* ✅ Admin-only Add2Database button (hidden after success) */}
...
//         {/* ✅ Admin-only AddSRToDatabase button (after requestSettlement confirmed, hidden after saved) */}
//         {showAddSRToDatabaseButton ? (
//           <div className="row" style={{ justifyContent: "center", marginTop: 10 }}>
//             <button
//               onClick={addSettlementHashToDatabase}
//               disabled={srDbBusy}
//               style={{ fontWeight: 900 }}
//             >
...
export default function MatchCard({ match, marketId, onMarketCreated }) {
  ...
  // “after the user bets, change to Get My Bet”
  ...
  return (
    <>
      ...
      >
        ...
        {/* ✅ Predict selection: WIN / LOST / DRAW */}
        <div className="row" style={{ alignItems: "center" }}>
          <select
            value={pick}
            onChange={(e) => setPick(e.target.value)}
        ...
        {/* My bet display (from getPrediction) */}
        {alreadyPredicted ? (
          <div
            className="small"
            style={{
