...
// ✅ Your Next/Vercel server base URL (separate app)
...
// ✅ helpers for your requireAdmin() scheme
function sha256HexBrowser(input) {
  // Server uses node crypto sha256 hex. In browser we do WebCrypto.
...
function randomNonce() {
  // simple, good enough nonce for this purpose
...
export default function MatchCard({ match, marketId, onMarketCreated }) {
  ...
  // =====================================================================
  // Admin: Create Market modal (your existing logic, unchanged UI)
  // =====================================================================
  const [showAdminPopup, setShowAdminPopup] = useState(false);
  const [adminAnimatingIn, setAdminAnimatingIn] = useState(false);
  const modalCardRef = useRef(null);
  ...
  // ✅ POST the match to your separate Next/Vercel server (Neon) using your requireAdmin() headers
  const postMatchToDb = async (createdIdStr) => {
    if (!API_BASE) throw new Error("Missing VITE_API_BASE_URL (server origin).");

    // must match MatchSchema
    const bodyObj = {
      marketId: Number(createdIdStr),
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // update parent UI state immediately
        onMarketCreated?.(match, createdIdStr);

        // then save to DB
        try {
          setAdminStatus(`✅ Market created (ID: ${createdIdStr}). Saving to DB...`);
          await postMatchToDb(createdIdStr);
          setAdminStatus(`✅ Market created (ID: ${createdIdStr}). Saved to DB ✅`);
        } catch (e) {
          console.error("DB save failed:", e);
          setAdminStatus(
            `✅ Market created (ID: ${createdIdStr}) but DB save failed: ${e?.message || e}`
          );
        }
  ...
  const onAdminCreateMarket = async () => {
    ...
    if (!API_BASE) {
      setAdminStatus("Missing VITE_API_BASE_URL (server origin).");
      return;
    }
    ...
    <>
      ...
      >
        ...
        <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
          ...
          {/* swap button based on whether SettlementRequested already happened */}
          {showRequestSettlementButton ? (
            <button
              onClick={onRequestSettlement}
              disabled={settleBusy}
