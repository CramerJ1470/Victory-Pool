...
function onLogTrigger(runtime2, log) {
  ...
  try {
    ...
    const market = Array.isArray(decoded) ? decoded[0] : decoded;
    runtime2.log(`[Step 2] Market creator: ${market.creator}`);
    runtime2.log(`[Step 2] Already settled: ${market.settled}`);
    runtime2.log(`[Step 2] Win Pool: ${market.totalWinPool.toString()}`);
    runtime2.log(`[Step 2] Lost Pool: ${market.totalLostPool.toString()}`);
    runtime2.log(`[Step 2] DRAW Pool: ${market.totalDrawPool.toString()}`);
    if (market.settled) {
    ...
    const outcomeValue = outcomeMap[parsed.result];
